...
func copyStarterScript(ctx context.Context, c testcontainers.Container) error {
	port, err := waitForMappedPort(ctx, c, publicPort, time.Minute, time.Second)
	if err != nil {
...
func waitForMappedPort(ctx context.Context, c testcontainers.Container, localPort nat.Port,
